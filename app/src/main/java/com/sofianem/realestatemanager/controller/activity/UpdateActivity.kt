    ...
    private fun retrieveData(id: Int) {
        mMyViewModel.allWordsLive.observe(this, androidx.lifecycle.Observer { list ->
            ...
            if (lstEst.type == "") {
                ...
                upload_type.text = lstEst.type; type = lstEst.type
            }
            if (lstEst.price == 0) {
                upload_tx_pric.text = "-"
            } else {
                upload_tx_pric.text = lstEst.price.toString() + "  $"; price = lstEst.price
            }
            if (lstEst.surface == 0) {
                upload_tx_surface.text = "-"
            } else {
                upload_tx_surface.text = lstEst.surface.toString() + "  Sq/ft"; surface =
                    lstEst.surface
            }
            if (lstEst.number_of_room == 0) {
                upload_room.text = "-"
            } else {
                upload_room.text = lstEst.number_of_room.toString(); number_of_room =
                    lstEst.number_of_room
            }
            if (lstEst.description == "") {
                upload_description.setText("-")
            } else {
                upload_description.setText(lstEst.description); description = lstEst.description
            }
            if (lstEst.personn == "") {
                upload_personn.text = "-"
            } else {
            ...
            var autocompleteFragment = Utils.configureAutoCompleteFrag(
            ...
            if (lstEst.date_begin.toInt().equals(3)) {
                upload_datebegin.text = "-"
            } else {
                date_begin = lstEst.date_begin; upload_datebegin.text =
                    Utils.convertToLocalB(lstEst)
            }

            if (lstEst.date_end.equals(8888888888)) {
                upload_dateend.text = "-"
            } else {
                date_end = lstEst.date_end; upload_dateend.text =
                    Utils.convertToLocalE(lstEst); cancel_button_upload.isVisible = true
            }

            upload_tx_pric.setOnClickListener { upload_rangebar_price.visibility = View.VISIBLE }
            upload_tx_surface.setOnClickListener {
                upload_rangebar_surface.visibility = View.VISIBLE
            }
        ...
        when (requestCode) {
            OPERATION_CAPTURE_PHOTO ->
                if (resultCode == Activity.RESULT_OK) {
                    val bitmap =
                        BitmapFactory.decodeStream(contentResolver.openInputStream(mUri)); saveImage(
                        bitmap
                    )
                }
    ...
    override fun deleteImage(IdImage: Int) {
        ...
        listimg.value?.forEach { img ->
            if (IdImage == img.imageId) {
                mMyViewModelForImages.deleteImage(img)}}
